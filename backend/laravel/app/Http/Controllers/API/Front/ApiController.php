...
class ApiController extends BaseController
{
    ...
    {
        $articles = Article::with('user:id,name,pfp', 'tags:name')->select('id', 'title', 'date_posted', 'image_url', 'user_id')->orderBy('date_posted', 'DESC')->limit($request->limit)->skip($request->offset)->where('is_published', 1)->get();
    ...
    public function get_article_homepage(Request $request)
    {
        ...
        $article = Article::with('user:id,name,pfp')->select('id', 'title', 'date_posted', 'image_url', 'price', 'liquidation_days', 'article_total_reads', 'article_total_shares', 'description', 'user_id')->where('id', $request->article_id)->where('is_published', 1)->get();
    ...
    {
        $articles = Article::with('user:id,name,pfp', 'tags:name')->select('id', 'title', 'date_posted', 'image_url', 'user_id')->orderBy('date_posted', 'DESC')->where('title', 'LIKE', '%' . $request->q . '%')->where('is_published', 1)->get();
