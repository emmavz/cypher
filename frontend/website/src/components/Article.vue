...
<template>
    <RouterLink :to="{name: 'article_homepage', params: {articleId: newArticle.article_id } }" v-if="newArticle" class="blog-post inline-flex flex-wrap justify-between">
        ...
        <div class="blog-post__left">
            ...
            <div class="blog-post__left__header__img">
                <img :src="newArticle.pfp" alt="" width="40">
            </div>
            <div class="pl-3">
                <h2 class="mb-1">{{ newArticle.article_title }}</h2>
                <div class="flex blog-post__left__header__author">
                    <span>{{ newArticle.name }}</span>
                </div>
            </div>
            </div>

            <ul :class="['categories mb-4', {'mt-6': 'stakes' in newArticle }]"  v-if="'hashtags' in newArticle">
                <li v-for="(tag, i) in newArticle.hashtags" :key="i">
            ...
            <div class="blog-post__left__stock" v-if="'total_invested' in newArticle">
                {{ newArticle.total_invested }} {{ this.currency }} Invested
            </div>
