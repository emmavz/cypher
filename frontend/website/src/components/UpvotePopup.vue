...
<template>
    ...
    <div>
        <button class="currency-tag currency-tag--opacity-70"
            @click.stop="votesStep = 1;emitter.emit('votes-step', 1)">Upvote</button>

        <div v-if="votesStep" class="votes-popup flex items-center" v-click-outside="closeUpvote">
            <div class="container">
                ...
                <template v-if="votesStep == 2">
                    <div class="flex justify-center items-center w-full">
                        ...
                        <div>
                            <form action="#" method="POST" @submit.prevent="upvote"
                                class="text-black flex items-center">
                                <div>
                                    <input type="text" name="votes" v-model="votes" v-on:blur="addVotesSuffix"
                                        v-on:focus="votes = removeVotesSuffix()" class="v-input mr-3.5 ml-6">
                                </div>
                                <div v-if="votes">
                                    <button type="submit" class="relative top-1 tick-icon"><img
                                            src="@/assets/img/tick-green-icon.svg" alt=""></button>
                                </div>
                            </form>
                        </div>
                    </div>
